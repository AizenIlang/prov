...
export class UserreportsComponent implements OnInit {
  ...
  displayedColumns = ['userName','email','type','hospitalMember','bloodType','gender','date','firstName','middleName','lastName','created'];
  ...
  async filterTestDate() {
    ...
    await this.UserList.snapshotChanges().subscribe(item => {
      ...
      item.forEach(element => {
        ...
        console.log(y);
        let userObject = this.reportService.userObjectParse(y);
        appointmentList.push(userObject);
      ...
      let selectedMembers = appointmentList.filter(
        m => new Date(m.created) >= new Date(start) && new Date(m.created) <= new Date(end)
      );
